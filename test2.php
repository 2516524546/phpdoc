...
error_reporting(E_ALL ^ E_WARNING);
       ...
       if ($zip->open ($file) === TRUE ) {
           for($i = 0; $i < $zip->numFiles; $i ++) {
               ...
               if (pathinfo ($entry,PATHINFO_BASENAME) == "document.xml") {
                   ...
                   $contentall_array = explode("%%",$contentall);
                   foreach ($contentall_array as $key_1 => $contentall) {
                     ...
                     $content[$key_1]['content'][]  = $cutname;
                     $content[$key_1]['cat'] = "";
                     $content[$key_1]['remark'] = "";

                     $catkey  =  substr($contentall,strpos($contentall,';')+1);
                     $content[$key_1]['cat']   =  substr($catkey,0,strpos($catkey,'&'));

                     $content[$key_1]['remark'] = substr($contentall,0,strpos($contentall,'&'));

                     //
                     //
                     // if ($content[$key_1]['remark'] == '') {
                     ...
                     //    $catkey = trim($catkey);
                     //    $content[$key_1]['oldname'] =  $catkey;
                     // }
                     $content[$key_1]['title'][]  = $content[$key_1]['remark'].'职业介绍' ;
                     $content[$key_1]['title'][]  = $content[$key_1]['remark'].'具体做什么' ;
                     $content[$key_1]['title'][]  = $content[$key_1]['remark'].'就业前景' ;
                     $content[$key_1]['title'][]  = $content[$key_1]['remark'].'对应的大学专业' ;
 ...
 foreach ($content as $key => $value) {
    ...
    if (strpos($value['cat'],'／')) {
      ...
      $filetxt2 =  "INSERT INTO `subject_vocation`(name,type,assess_id,create_at)  VALUES('{$value['remark']}','$cat1',$id,'$ctime');";
      $filetxt2 .=  "INSERT INTO `subject_vocation`(name,type,assess_id,create_at)  VALUES('{$value['remark']}','$cat2',$id,'$ctime');";
    }elseif (strpos($value['cat'],'_')) {
      $cat1 =   substr($value['cat'],0,strpos($value['cat'],'_'));
      $cat2 =   substr($value['cat'],strpos($value['cat'],'_')+1);
      $filetxt2 =  "INSERT INTO `subject_vocation`(name,type,assess_id,create_at)  VALUES('{$value['remark']}','$cat1',$id,'$ctime');";
      $filetxt2 .=  "INSERT INTO `subject_vocation`(name,type,assess_id,create_at)  VALUES('{$value['remark']}','$cat2',$id,'$ctime');";
    }elseif (strpos($value['cat'],'/')) {
      $cat1 =   substr($value['cat'],0,strpos($value['cat'],'/'));
      $cat2 =   substr($value['cat'],strpos($value['cat'],'/')+1);
      $filetxt2 =  "INSERT INTO `subject_vocation`(name,type,assess_id,create_at)  VALUES('{$value['remark']}','$cat1',$id,'$ctime');";
      $filetxt2 .=  "INSERT INTO `subject_vocation`(name,type,assess_id,create_at)  VALUES('{$value['remark']}','$cat2',$id,'$ctime');";
    }
      ...
      $filetxt2 =  "INSERT INTO `subject_vocation`(name,type,assess_id,create_at)  VALUES('{$value['remark']}','{$value['cat']}',$id,'$ctime');";
    }
    $txt = $filetxt."\n".$filetxt2;
